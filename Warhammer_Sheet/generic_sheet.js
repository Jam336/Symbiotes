...
function createDiceRoll(clickElement, inputElement) {
    ...
    {
        if (clickElement.dataset.modtype == null) {
            modifierString = inputElement.value >= 0 ? "+" + inputElement.value : inputElement.value;
            rolltype = "mod";
        }

        if (clickElement.dataset.modtype == "dieCount") {
            rolltype = "Dcount";
            modifierString = inputElement.value >= 1 ? inputElement.value : 1;
        }
    ...
    if (rolltype == "mod")
    { 
        roll = `${clickElement.dataset.diceType}${modifierString == '+' ? '' : modifierString}`
    }

    if (rolltype == "Dcount")
    {
        roll = `${modifierString == '0' ? '' : modifierString}${clickElement.dataset.diceType}`
    }
